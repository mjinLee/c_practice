...
int main_struct_project(void) {
	...
	while (1) {
		...
		int selected = rand() % 5; //0-4 사이의 숫자를 반환
		printCat(selected); // 뽑은 고양이 정보 출력
		collection[selected] = 1; // 고양이 뽑기 처리

		int collectAll = checkCollection(); // 고양이 다 모았는지 확인
		if (collectAll == 1) {
			break;
		}
	}
...
void initCats() {
	...
	cats[3].name = "치즈";
	cats[3].age = 4;
	cats[3].character = "사람을 잘따름";
	cats[3].level = 4;

	cats[4].name = "동글이";
	cats[4].age = 2;
	cats[4].character = "모험심이 강함";
	cats[4].level = 5;
}
