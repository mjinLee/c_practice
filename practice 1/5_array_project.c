...
int main_array_project(void) {
	...
	for (int i = 1; i <= 3; i++) {
		...
		} while (cntShowBottle == preCntShowBottle); // 병 갯수 서로 다르게
		...
		for (int j = 0; j < cntShowBottle; j++) {
			...
			if (bottle[randBottle] == 0) {
				...
				if (randBottle == treatment) {
					isIncluded = 1; // 1 이면 포함
				}
			}
			//이미 선택된 병이면, 중복으로 다시 선택
			else {
				j--; //이미 for문에서 j++ 되었으므로 j--해줘야함
			}
		}
		...
		printf("\n ...계속하려면 아무키나 누르세요  ");
		getchar(); //사용자가 키를 누를 때까지 대기
	}

	printf("\n\n발모제는 몇 번일까요? ");
	scanf_s("%d", &answer);

	if (answer == treatment + 1) //treatment 값이 0-3 까지이므로 +1
		printf("\n >> 정답입니다!\n");
	else
		printf("\n >> 땡 ! 틀렸어요. 정답은 %d 입니다\n", treatment + 1);
